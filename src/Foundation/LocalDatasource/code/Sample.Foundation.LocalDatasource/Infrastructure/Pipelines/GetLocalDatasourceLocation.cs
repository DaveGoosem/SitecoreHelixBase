...
using Sample.Foundation.LocalDatasource.Extensions;
using Sitecore.Pipelines.GetRenderingDatasource;
using Sitecore.SecurityModel;
using Sitecore;

namespace Sample.Foundation.LocalDatasource.Infrastructure.Pipelines
{
    //https://github.com/Sitecore/Sitecore.HabitatHome.Corporate/tree/master/src/Foundation/LocalDatasource
    public class GetLocalDatasourceLocation
    {
        public void Process(GetRenderingDatasourceArgs args)
        {
            ...
            if (datasource == null || !datasource.Checked) return;
        ...
        private Item GetConfiguredLocalDatasourceFolder(Item contextItem, Item datasourceTemplate)
        {
            ...
            using (new BulkUpdateContext())
            {
                ...
                if (localDatasourceFolder == null) return null;
        ...
        {
            if (datasourceTemplate == null) return;
            var insertOptions = localDatasourceFolder[FieldIDs.Branches];
            //Is the datasource template already on the insert options?
            if (insertOptions.IndexOf(datasourceTemplate.ID.ToString(), StringComparison.InvariantCultureIgnoreCase) > -1) return;
